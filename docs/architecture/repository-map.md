...
- **[`aerobeat-input-core`](https://github.com/AeroBeat-Workouts/aerobeat-input-core)**: shared gameplay-facing input abstractions, normalized provider payloads, provider contracts, and the canonical UI interaction contract / native 2D bridge lane
...
- **`aerobeat-input-core`** owns normalized input/provider contracts that gameplay and UI can both consume, including the canonical UI interaction contract and native 2D bridge path.
- **`aerobeat-ui-core`** owns UI-facing contracts and reusable base behavior, but not concrete spatial pointer extraction.
- **`aerobeat-spatial-ui-core`** is a shared **helper layer** for spatial/world UI adapters: reusable surface descriptors, target-resolution helpers, projection helpers, and hover/capture policy scaffolding.
- **`aerobeat-spatial-ui-mouse`** is the first concrete spatial provider lane for desktop pointer input. It packages the mouse-backed implementation that consumes the shared helper layer and publishes into the existing input contract.
...
2. `aerobeat-input-core` provides the canonical UI interaction contract and native 2D bridge lane
3. `aerobeat-spatial-ui-core` provides reusable helper-layer pieces that concrete spatial providers can share
4. a concrete provider package such as `aerobeat-spatial-ui-mouse` supplies the actual mouse-backed pointer extraction/runtime behavior
